...
function minimize (dfa) {
  ...
  */
  ...
  // stateToGroupMap has just been patched to map the accept states back to themselves
  ...
  for(i=0, ii=dfa.accept.length; i<ii; ++i) {
    ...
    if(newAccept.indexOf(temp) < 0) {
      ...
      // Figure out what accept states were merged together
      merged[dfa.accept] = temp
    }
  }
  ...
  // Reverse stateToGroupMap so we have a mapping from group to original states
  for(k in stateToGroupMap) {
    if(dfa.transitions[k]) {
      if(reverseMap[k] == null) {
        reverseMap[k] = [stateToGroupMap[k]]
      }
      else {
        reverseMap[k].push(stateToGroupMap[k])
      }
    }
  }
