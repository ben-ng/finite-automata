...
Fragment.prototype.minimize = function minimize () {
  var def = _minimize(this.copy())
    , states = Object.keys(def.transitions).sort()
    , i = 0
    , ii = states.length
    , j = 0
    , jj = 0
    ...
    , stateIndex
    , stateMap = {}
    , newTransitions = new Array(ii)

  // Add all states to the map, assigning them to integers
  for(; i<ii; ++i) {
    stateMap[states[i]] = i
  }

  for(state in def.transitions) {
    stateIndex = stateMap[state]
    newTransitions[stateIndex] = []

    for(j=0, jj=def.transitions[state].length; j<jj; j+=2) {
      newTransitions[stateIndex].push(def.transitions[state][j], stateMap[def.transitions[state][j + 1]])
    }
  }

  def.initial = stateMap[def.initial]

  for(j=0, jj=def.accept.length; j<jj; ++j) {
    def.accept[j] = stateMap[def.accept[j]]
  }

  def.transitions = newTransitions
